    ...
    class Program
    {
        ...
        static TabPage BuildAboutTab()
        {
            ...
            return new TabPage("About", new ScrollViewer(new VStack(
                ...
                "  NoSQL:   MongoDB, RavenDB, Cosmos DB",
                ...
                "  Modbus, OAuth, Camera, IR, NFC",
                "",
                new Rule("AI / LLM"),
                "  Multi-provider LLM (Claude, OpenAI, Gemini, Ollama, Azure, etc.)",
                "  Coding/media/task agents, orchestration, resilience",
                "",
                new Rule("Cross-Cutting"),
                "  Validation, Caching (Memory/Redis/Hybrid), Security (PBKDF2/AES/JWT/BCrypt/Vault)",
                "  Patterns (UoW/SoftDelete/Audit/Paging), Migrations, Sync",
                "  Background Jobs (SQL/ElasticSearch/MongoDB/RavenDB/JSON/Redis/CosmosDB)",
                "  Message Queue (InMemory/MQTT/Redis), Event Bus (In-Process/Distributed/Outbox)",
                "  Messaging (Email/SMTP, Razor templates, SMS, Push), Storage (Local, Azure Blob)",
                "  Telemetry (Store Metrics/Tracing, Correlation ID, OpenTelemetry)",
                "  Workflow (State machines, guards, actions, Mermaid/DOT)",
                "  CQRS, Rules Engine, Data Processors (XML/CSV/HTTP/ZIP)",
                "  Health Checks (Disk/Memory/SQL/ES/MongoDB/RavenDB/InfluxDB/Redis/MQTT/SMTP/Vault/Azure)",
                "  Localization (CLDR, DB, entity-level), Time (Calendar, holidays, working hours)",
                "  Serialization (System.Text.Json, Newtonsoft, MessagePack, Protobuf)",
                "",
