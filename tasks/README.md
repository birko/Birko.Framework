...
> ⚠ **Feature drift (3 groups, 85 items)** — **DV9 re-measured 2026-09-09**; the other two counts are
> carried from 2026-09-04 and are labelled as such.
> **DV9 ×63** — **re-measured 2026-09-09** at TASK-308's close: 188 tasks carry
> `feature: FEATURE-014` and its `decisions.md` `→ Tasks` column lists 125 of them, so the ledger does
> not know about 63 of its own tasks (×31 on 2026-09-04, ×59 on 2026-09-08, ×62 earlier today; the one
> added since is TASK-331). Measured rather than incremented each time, because the count carried
...
> `bulk-filter-operations` (substantive) and `filter-expression-translation` (its pointer requirement) —
> along with `store-lazy-initialization`, whose globs the same diff reached and whose body was left
> **deliberately unchanged** because the change contradicts nothing it asserts.
> `filter-expression-translation` was then **substantively rewritten later the same day** by
> [[TASK-308]]: two requirements that documented its defects *as shipped behaviour* were replaced and
> four added, 44 → 48. So DV7 is very likely **×1** (`unit-of-work-and-transactions`, subject of
> [[TASK-325]]/[[TASK-327]] and therefore about to change again) — but that is arithmetic on a stale
> total rather than a measurement, and it is not reported as one. DV8/DV10/DV11 were clean at the last measurement: all
...
_Generated 2026-09-09 (TASK-308 close). Run `/tasks triage` to refresh. **Do not hand-edit** — changes will be overwritten._
...
- **EPIC-014** Code review — audit remediation — in-progress (105/192 tasks done)
