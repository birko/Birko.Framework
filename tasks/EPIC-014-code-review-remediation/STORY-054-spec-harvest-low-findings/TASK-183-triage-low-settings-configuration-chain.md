...
findings: [SH-L250, SH-L251, SH-L252, SH-L253, SH-L254, SH-L255, SH-L256, SH-L257, SH-L258, SH-L259, SH-L260, SH-L261, SH-L262, SH-L263, SH-L264, SH-L265, SH-L266, SH-L267]
pr: null
github-issue: null
jira-key: null
---

# Triage the 18 low spec-harvest findings in `settings-configuration-chain`
...
or by [[fix-next]], so the whole backlog was invisible to every picker. This task owns the **18** findings
that fall in the `settings-configuration-chain` area.

**Findings:** `SH-L250`–`SH-L267` (contiguous), detailed in
[`SPEC-HARVEST-FINDINGS-2026-07-30.md`](../SPEC-HARVEST-FINDINGS-2026-07-30.md) § Low severity →
`### area: settings-configuration-chain`, lines 4546–4655.

**The contract under review** is specced in [`docs/specs/settings-configuration-chain.md`](../../../docs/specs/settings-configuration-chain.md), harvested from
13 source globs — `../Birko.Configuration/Settings.cs`, `../Birko.Data.SQL/Stores/SqlSettings.cs`, `../Birko.Data.SQL.MSSql/Stores/MSSqlSettings.cs`, and 10 more. Every one of
...
`docs/specs/settings-configuration-chain.md` lying until `/specs regen settings-configuration-chain` runs, and **that spec diff is the fix's evidence**.

## Acceptance criteria

- [ ] All 18 findings are marked **confirmed**, **confirmed-narrower**, or **refuted** against the code,
...
- [ ] Any behavioural fix is followed by `/specs regen settings-configuration-chain`, with the spec diff reviewed as the change's
      evidence
...
- The other 369 low findings — they belong to the other 21 per-area tasks under [[STORY-054]].
- High-severity findings in this area — [[STORY-051]] owns those.
- Medium-severity findings in this area — [[TASK-164]] owns them. Where a fix closes both, do it once and
