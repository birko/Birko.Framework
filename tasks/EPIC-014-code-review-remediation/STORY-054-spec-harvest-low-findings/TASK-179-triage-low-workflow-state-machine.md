...
findings: [SH-L367, SH-L368, SH-L369, SH-L370, SH-L371, SH-L372, SH-L373, SH-L374, SH-L375, SH-L376, SH-L377, SH-L378, SH-L379, SH-L380, SH-L381, SH-L382, SH-L383, SH-L384, SH-L385, SH-L386, SH-L387]
pr: null
github-issue: null
jira-key: null
---

# Triage the 21 low spec-harvest findings in `workflow-state-machine`
...
or by [[fix-next]], so the whole backlog was invisible to every picker. This task owns the **21** findings
that fall in the `workflow-state-machine` area.

**Findings:** `SH-L367`–`SH-L387` (contiguous), detailed in
[`SPEC-HARVEST-FINDINGS-2026-07-30.md`](../SPEC-HARVEST-FINDINGS-2026-07-30.md) § Low severity →
`### area: workflow-state-machine`, lines 5262–5388.

**The contract under review** is specced in [`docs/specs/workflow-state-machine.md`](../../../docs/specs/workflow-state-machine.md), harvested from
8 source globs — `../Birko.Workflow/**/*.cs`, `../Birko.Workflow.SQL/**/*.cs`, `../Birko.Workflow.ElasticSearch/**/*.cs`, and 5 more. Every one of
...
`docs/specs/workflow-state-machine.md` lying until `/specs regen workflow-state-machine` runs, and **that spec diff is the fix's evidence**.

## Acceptance criteria

- [ ] All 21 findings are marked **confirmed**, **confirmed-narrower**, or **refuted** against the code,
...
- [ ] Any behavioural fix is followed by `/specs regen workflow-state-machine`, with the spec diff reviewed as the change's
      evidence
...
- The other 366 low findings — they belong to the other 21 per-area tasks under [[STORY-054]].
- High-severity findings in this area — [[STORY-051]] owns those.
- Medium-severity findings in this area — [[TASK-172]] owns them. Where a fix closes both, do it once and
