...
severity: mixed
finding-count: 865
finding-ids: SH-H001 … SH-H057, SH-M001 … SH-M421, SH-L001 … SH-L387
---
...
**0 / 865 closed.** 15 of the 57 high findings are hand-verified (13 CONFIRMED, 2 of those with corrected
...
`FINDINGS.md` holds all 865 with file:line and reasoning, grouped by severity then area. Severity is by
...
- **No area is absent.** All 22 previously-capped areas are swept uncapped, and every agent self-reported
  `sweptToExhaustion: true` — the agents' own claim, not independently checked.
- The three areas swept last — `serialization` (24), `llm-provider-and-agents` (51) and
  `validation-and-rules` (41) — produced **no high findings between them**. Risk is not spread evenly: the
  highs concentrate in the data-access, tenant and security areas, which is also where this repo's own
  CHANGELOG history sits.
